 ...
 * @size: array lenght
 */
void sortbits(int arr[], int low, int nelemnt, int order, int size)
{
	int mid;
	...
	if (nelemnt > 1)
	{
		...
		mid = nelemnt / 2;
		sortbits(arr, low, mid, 1, size);
		sortbits(arr, low + mid, mid, 0, size);
...
void bitonic_sort(int *array, size_t size)
{
	...
	if (!array || size < 2)
		return;
	sortbits(array, 0, size, order, size);
}
